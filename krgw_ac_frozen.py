...
from pyscf.pbc.mp.kmp2 import get_nocc, get_nmo, get_frozen_mask, padded_mo_energy, padded_mo_coeff
...
#        nmo = len([3,4,5,6,7])
    ...
    _v_mf = np.array(mf.get_veff()) - np.array(mf.get_j(dm_kpts=dm))
    v_mf = np.zeros((nkpts, nmo, nmo), dtype=_v_mf[0].dtype)
    for k in range(nkpts):
        v_mf[k] = reduce(numpy.dot, (mo_coeff[k].T.conj(), _v_mf[k], mo_coeff[k]))
    ...
    _vk = rhf.get_veff(gw.mol,dm_kpts=dm) - rhf.get_j(gw.mol,dm_kpts=dm)
    vk = np.zeros((nkpts, nmo, nmo), dtype=_vk[0].dtype)
    for k in range(nkpts):
        vk[k] = reduce(numpy.dot, (mo_coeff[k].T.conj(), _vk[k], mo_coeff[k]))
        ...
        for p in orbs:
            if gw.linearized:
                ...
                def quasiparticle(omega):
                    ...
                    return omega - mf_mo_energy[kn][p] - (sigmaR.real + vk[kn,p,p].real - v_mf[kn,p,p].real)
                try:
                    e = newton(quasiparticle, mf_mo_energy[kn][p], tol=1e-6, maxiter=100)
                    mo_energy[kn,p+frozen_nocc] = e
                except RuntimeError:
                    conv = False
